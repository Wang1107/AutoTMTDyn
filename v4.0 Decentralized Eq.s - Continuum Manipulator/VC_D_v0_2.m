...
% one module symmetric VC discrete, n_seg segments
% not working in 3Do due to no dynamic for phi direction
...
l_b = 44e-3 ; l_fs = 10e-3 ; l_base = 10e-3 ;
l_e = l_b +  l_fs ;
...
n_seg = 1 ; % number of segments in the discrete VC
eps = 1e-3 ; % singularity prevention

tspringCoeff = n_seg / ( lambdal ^ 2 * l_b / E / a_S0 ) ; % translational, delta l
tspringInit = 0;
tdampViscous = 1e-3 / n_seg ;
tdampCoulomb = 0;
rspringCoeff = n_seg / ( lambdal ^ 3 / E / I_1 ) ; % rotational, kappa
rspringInit = 0;
rdampViscous = 4e-5 / n_seg; % 4e-5 for Cosserat
rdampCoulomb = 0;
...
for i = 1 : n_seg
    ...
    body(i).type = 'CC_EB' ; % 'rigid', 'CC_EB', 'VC', default: rigid
                            % VC discrete is CC_EB with many segments
                            % curvilinear frame: [d_y d_x d_z] ~ [ y x z ]
    body(i).CC_EB.m = m_b / l_b / n_seg ; % mass or mass/length
    body(i).CC_EB.I = I_b / l_b / n_seg ; % inertia around COM expressed in local frame
    body(i).CC_EB.l_com = [ 0 0 l_b/2/n_seg ] ; % COM position vector w.r.t & expressed in local frame
    body(i).CC_EB.tip = [ l_b/n_seg 0 0 ] ; % tip position vector w.r.t & expressed in local curvilinear frame
    body(i).CC_EB.par = [ inf 0 0 0 inf inf ] ; % put 'inf' for CC_EB states to be dof and values for fixed directions
                                        % [ epsilon_x l epsilon_z phi kappa_x kappa_z ] in curvilinear frame for CC & VC
                                        % epsilon is for shear, not working with shear at the moment
    body(i).CC_EB.stiff = [ tspringCoeff 0 0 0 rspringCoeff rspringCoeff ] ; % curvilinear translational and rotational stiffness: [ t n b tr nr br ]
                                                                             % set to Ea/l, EI/l or GJ/l from EB relation
    body(i).CC_EB.viscdamp = [ tdampViscous 0 0 0 rdampViscous rdampViscous ] ; % curvilinear translational and rotational damping: [ t n b tr nr br ]
    body(i).CC_EB.couldamp = [ tdampCoulomb 0 0 0 rdampCoulomb rdampCoulomb ] ; % curvilinear translational and rotational damping: [ t n b tr nr br ]
    body(i).CC_EB.dof(1).equal2 = [] ; % equal to another CC_EB parameter [ bodyNumber dofNumberInBody ]
    body(i).CC_EB.dof(1).init = l_b / n_seg ; % flexible body states' initial condition
    body(i).CC_EB.dof(1).spring = [ 0 0 ] ;
    body(i).CC_EB.dof(1).damp = [ 0 0 ] ;
    body(i).CC_EB.dof(2).init = eps ;
    body(i).CC_EB.dof(2).spring = [ 0 0 ] ;
    body(i).CC_EB.dof(2).damp = [ 0 0 ] ;
    body(i).CC_EB.dof(3).init = eps ;
    body(i).CC_EB.dof(3).spring = [ 0 0 ] ;
    body(i).CC_EB.dof(3).damp = [ 0 0 ] ;

end

% % for hyperbolic solution only
% % for CC it is similar to having only one CC segment
% for i = 2 : n_seg
%     body(i).CC_EB.dof(1).equal2 = [ 1 1 ] ; % equal to another CC_EB parameter [ bodyNumber dofNumberInBody ]
%     body(i).CC_EB.dof(1).init = l_b / n_seg ; % flexible body states' initial condition
%     body(i).CC_EB.dof(1).spring = [ 0 0 ] ;
%     body(i).CC_EB.dof(1).damp = [ 0 0 ] ;
%     body(i).CC_EB.dof(2).equal2 = [ 1 2 ] ;
%     body(i).CC_EB.dof(2).init = eps ;
%     body(i).CC_EB.dof(2).spring = [ 0 0 ] ;
%     body(i).CC_EB.dof(2).damp = [ 0 0 ] ;
%     body(i).CC_EB.dof(3).equal2 = [ 1 3 ] ;
%     body(i).CC_EB.dof(3).init = eps ;
%     body(i).CC_EB.dof(3).spring = [ 0 0 ] ;
%     body(i).CC_EB.dof(3).damp = [ 0 0 ] ; 
% end
...
i = n_seg ;
body(i+1).type = 'rigid' ;
body(i+1).rigid.m = m_e ;
body(i+1).rigid.I = I_e ;
body(i+1).rigid.l_com = [ 0 0 (l_b+l_fs)/2 ] ;
body(i+1).rigid.tip = [ 0 0 l_b+l_fs ] ;
...
joint(1).tr(1).trans = [ 0 0 l_base ] ; % position vector w.r.t. & exp. in prev. joint local Cartesian frame
joint(1).tr(1).rot = [ 0 0 ] ; % position vector w.r.t. & exp. in prev. joint local frame
                               % both can be left empty
joint(1).tr2nd = [] ; % definitions

for i = 2 : n_seg
    joint(i).first = i-1 ; % first body number, 0 for inertial frame
    joint(i).second = i ; % second body number, 0 for inertial frame
                          % define second body local frame
    joint(i).tr(1).trans = [ l_b/n_seg 0 0 ] ; % position vector w.r.t. & exp. in prev. joint local curvilinear frame
    joint(i).tr(1).rot = [ 0 0 ] ; % position vector w.r.t. & exp. in prev. joint local frame
                                   % both can be left empty
    joint(i).tr2nd = [] ; % definitions
end

% a  module as external load
i = n_seg ;
joint(i+1).first = i ;
joint(i+1).second = i+1 ;
joint(i+1).tr(1).trans = [ l_b/n_seg 0 0 ] ; % this is mid-base sensor position in local curvinilinear frame
joint(i+1).tr(1).rot = [ 0 0 ] ;

exload = [] ; % definition
exload(1).body = i ; % point of contact bosdy number
exload(1).tr(1).trans = [ l_b/n_seg 0 0 ] ; % point translation and rotation vector w.r.t & exp. in local Cartesian frame
...
par.p_rho = p_rho ;
par.t0 = 0 ; % NOT WORKING for nonzero
par.dt = 30 ; % final tmie: 218
par.stepT = 25e-3 ; % time step for sensor readings is 0.025
par.symV = 1 ; % value for lambdal
par.l_b = l_b ; par.phi_b = phi_b ;
par.d_p = d_p ; par.a_p = a_p ; par.n_seg = n_seg ;
[ t , z , tfinal ] = SimEoM_VC_D_v0_1( qf , uf , lambdaf , sf , qf0 , par ) ;
% plot
