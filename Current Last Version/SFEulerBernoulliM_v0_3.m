...
tdampViscous = 1e-1;
tdampCoulomb = 0;
rspringCoeff = 1 / ( lambdal ^ 3 * l_b / E / I_1 ) ; % rotational
rspringCoeff = 0.1 * rspringCoeff ; % corrected
rspringInit = 0;
rdampViscous = 5e-4; % 4e-5 for Cosserat
rdampCoulomb = 0;

eps = 1e-9 ; % singularity prevention
...
body(1).m = m_b / l_b ; % mass or mass/length
body(1).I = I_b / l_b ; % inertia around COM expressed in local frame
body(1).l_com = [ 0 0 l_b/2 ] ; % COM position vector w.r.t & expressed in local frame
body(1).tip = l_b ; % tip position vector w.r.t & expressed in local frame
body(1).curve.stiff = [ tspringCoeff 0 0 0 0 rspringCoeff ] ; % curvilinear translational and rotational stiffness: [ t n b tr nr br ]
                                                              % set to Ea/l, EI/l or GJ/l from EB relation
body(1).curve.viscdamp = [ tdampViscous 0 0 0 0 rdampViscous ] ; % curvilinear translational and rotational damping: [ t n b tr nr br ]
body(1).curve.couldamp = [ tdampCoulomb 0 0 0 0 rdampCoulomb ] ; % curvilinear translational and rotational damping: [ t n b tr nr br ]
body(1).curve.par = [ inf inf 0 ] ; % put 'inf' for curve states to be dof and values for fixed directions
                                    % [ l kappa phi ] for EB-CC, 3*body(i).curve.slice for VC
body(1).curve.dof(1).equal2 = [] ; % equal to another curve parameter [ bodyNumber dofNumberInBody ]
body(1).curve.dof(1).init = l_b ; % flexible body states' initial condition
                                  % [ l kappa phi ] for EB-CC, 3*body(i).curve.slice for VC
body(1).curve.dof(1).spring = [ 0 0 ] ;
body(1).curve.dof(1).damp = [ 0 0 ] ;
body(1).curve.dof(2).init = eps ;
body(1).curve.dof(2).spring = [ 0 0 ] ;
body(1).curve.dof(2).damp = [ 0 0 ] ;

body(2).type = 'rigid' ;
body(2).tip = [ 0 0 l_b+l_fs ] ;
body(2).m = m_e ;
body(2).I = I_e ;
body(2).l_com = [ 0 0 l_e/2 ] ;
body(2).tip = [ 0 0 l_e ] ;
...
joint(1).tr(1).trans = [ 0 0 l_fs ] ; % position vector w.r.t. & exp. in prev. joint local frame
...
joint(2).first = 1 ;
joint(2).second = 2 ;
joint(2).tr(1).trans = l_b+l_fs ; % this is mid-base sensor position
joint(2).tr(1).rot = [ 0 0 ] ;

exload = [] ; % definition
exload(1).body = 1 ; % point of contact body number
exload(1).tr(1).trans = l_b+l_fs ; % point translation and rotation vector w.r.t & exp. in local frame
...
par.p_rho = p_rho ;
par.t0 = 0 ; par.dt = 50 ; % final tmie: 218
...
[ t , z , tfinal ] = SimEoM_EB_v0_3( qf , uf , lambdaf , sf , qf0 , par ) ;
% plot
...
for i = 1 : n_p
    ...
    psi(i) = phi_b + atan2( ( sqrt( 3 ) * ( p_i(2) + p_i(3) - 2 * p_i(1) ) ) , ( 3 * ( p_i(2) - p_i(3) ) ) ) - pi / 2 ;
    T_psi = [ cos( psi(i) ) -sin( psi(i) ) 0 ; sin( psi(i) ) cos( psi(i) ) 0 ; 0 0 1 ] ;
    tmp = ( T_psi * rjtip(1,4:6,i)' )' ; % tip for 1st body
    rjt3d(:,:,i) = tmp ;
